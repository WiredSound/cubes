...
const float FONT_DRAW_SCALE_FACTOR = 4.0;
...
namespace render {
    ...
    void DebugTextRenderer::draw() const {
        ...
        const float spacing = static_cast<float>(FONT_CHARACTER_HEIGHT) / static_cast<float>(window_height) * FONT_DRAW_SCALE_FACTOR;

        float draw_y = 0.9;

        for(const auto& pair : text_meshes) {
            program.set_uniform("line_position", glm::vec2(-0.98, draw_y));
